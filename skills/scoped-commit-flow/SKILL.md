...
3. Choose the smallest honest boundary:
   - one finished file when the work is file-by-file
   - one logical change when files are coupled
...
For launched target projects, publish mode `auto` means the helper commits and pushes coherent repo-visible non-private work after each finished file or logical chunk. For this launcher repo, keep `--no-push` unless the user explicitly asks to push.
